...
################################################################################
def align_seqs_scores_1hot(seq_vecs, seq_scores, atac_scores, sort=True):
    if sort:
    ...
    for header in seq_headers:
        ...
        train_atac_scores.append(atac_scores[header])

    # stack into matrices
    train_seqs = np.vstack(train_seqs)
    train_scores = np.vstack(train_scores)
    train_atac_scores = np.vstack(train_atac_scores)

    return train_seqs, train_scores, train_atac_scores
...
#  train_scores:  Matrix with score vector rows.
################################################################################
def load_data_1hot(fasta_file, scores_file, atac_file, extend_len=None, mean_norm=True, whiten=False, permute=True, sort=False):
    # load sequences
    seq_vecs = hash_sequences_1hot(fasta_file, extend_len)

    # load scores
    seq_scores, cols_target = hash_scores(scores_file)

    # load scores
    atac_scores, cols_atac = hash_scores(atac_file)

    #make sure cell lines are in the same order
    assert(cols_target == cols_atac)

    # align and construct input matrix
    train_seqs, train_scores, train_atac_scores = align_seqs_scores_1hot(seq_vecs, seq_scores, atac_scores, sort)
